         ...
         IF (MAXVAL(nbo_occ).GT.bond_cutoff) THEN
            ...
            DO inbo=1,nnbo
               ...
               IF (MAX(polarization,1.d0-polarization).LE.polarization_thresh) THEN
                  ...
                  !proj_dummy = P
                  !P = MATMUL(Pi,P)
                  !CALL GEMM(Pi,proj_dummy,P,'N','N',1.d0,0.d0)
...
50    ENDDO
      ...
      !CALL GEMM(inp%rho0(:,:,1),P,proj_dummy,'N','N',1.d0,0.d0)
      !CALL GEMM(P,proj_dummy,inp%rho0(:,:,1),'N','N',1.d0,0.d0)
    ...
    SUBROUTINE do_1c_search(inp,lpairs,nnho,nbondorlp,rhoat,nbo_coeff,nbo_occ,nho_coeff,ispin)
      ...
      DO ibondorlp=1,nbondorlp
         ...
         Pdummy=P
         !P=MATMUL(Pi,P)
         CALL GEMM(Pi,Pdummy,P,'N','N',1.d0,0.d0)
      ...
      !inp%rho0(:,:,1) = MATMUL(P,(MATMUL(inp%rho0(:,:,1),P)))
      CALL GEMM(inp%rho0(:,:,1,ispin),P,Pdummy,'N','N',1.d0,0.d0)
      CALL GEMM(P,Pdummy,inp%rho0(:,:,1,ispin),'N','N',1.d0,0.d0)
...
30       ENDDO
    ...
    FUNCTION schmidt_orthog_matrix(S,n) RESULT(Os)
      ...
      CALL GEMM(Sinv,S(1:n,n+1:nbasis),proj,'N','N',1.d0,0.d0)
